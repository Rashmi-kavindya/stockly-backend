...
report_gen = ReportGenerator(get_db_connection)
...
def generate_report():
    ...
    try:
        ...
        if report_type not in ['sales', 'inventory']:
            return jsonify({'success': False, 'error': 'Unsupported report type. Use "sales" or "inventory"'}), 400

        # Generate report using ReportGenerator
        try:
            if report_type == 'sales':
                buffer = report_gen.generate_sales_report(month, username)
            else:  # inventory
                buffer = report_gen.generate_inventory_report(month, username)
