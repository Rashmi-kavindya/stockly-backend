...
try:
    ...
    # reportlab may not be installed in dev environment; generation endpoint will raise helpful error
    canvas = None
    letter = None
...
@jwt_required()
def generate_report():
    """Generate a simple PDF report and return as downloadable file.
    Expected JSON: { report_type: 'sales'|'inventory'|'stock', month: 'January'|'All'|<number> }
    """
    if canvas is None:
        return jsonify({'success': False, 'error': 'PDF generation library (reportlab) is not installed'}), 500
    ...
    try:
        ...
        # Build basic report content depending on type
        conn = get_db_connection()
        cur = conn.cursor()
        ...
        if report_type == 'sales':
            if str(month).lower() == 'all' or month == 'All':
                cur.execute("""
                ...
                # Expect month as month name or number
                try:
                    month_idx = int(month)
                except Exception:
                    months_map = {m.lower(): idx+1 for idx, m in enumerate(['January','February','March','April','May','June','July','August','September','October','November','December'])}
                    month_idx = months_map.get(str(month).lower(), None)
            ...
            return jsonify({'success': False, 'error': 'Unsupported report type'}), 400
        ...
        c.drawString(72, height - 90, f"Generated by: {get_jwt_identity()} on {datetime.now().strftime('%Y-%m-%d %H:%M')}")
        ...
        if report_type == 'sales':
            ...
            c.drawString(320, y, 'Total Sold')
        else:
            c.drawString(72, y, 'Item')
            c.drawString(320, y, 'Stock')
        y -= 20
        c.setFont('Helvetica', 10)
        ...
        for r in rows:
            ...
            try:
                ...
                value = str(r.get('total', r.get('stock', '')))
        ...
        filename = f"{report_type}_report_{str(month).replace(' ', '_')}.pdf"
